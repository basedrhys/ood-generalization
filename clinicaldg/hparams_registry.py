...
def _hparams(algorithm, dataset, random_seed):
    ...
    if algorithm in ['DANN', 'CDANN']:
        ...
        _hparam('rvp_lambda', 1e4, lambda r: 10**r.uniform(-1, 5))
        _hparam('rvp_penalty_anneal_iters', 500, lambda r: int(10**r.uniform(0, 4)))

    elif algorithm == "SD":
        _hparam('sd_reg', 0.1, lambda r: 10**r.uniform(-5, -1))
        
    elif algorithm == "IGA":
        _hparam('iga_lambda', 1e4, lambda r: 10**r.uniform(-1, 5))
    ...
    if dataset in SMALL_IMAGES:
        _hparam('batch_size', 128, lambda r: int(2**r.uniform(6, 9)) )
    elif algorithm == 'ARM':
